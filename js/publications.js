...
(async function init(){
  const data=await getData(); const upd=document.getElementById('pub-updated');
  const search=document.getElementById('search'), year=document.getElementById('yearFilter'), sort=document.getElementById('sortBy');
  ...
  let pubs=data.publications; if(upd&&data.metrics) upd.textContent='Updated: '+(data.metrics.lastUpdated||'');
  years(pubs).forEach(y=>{ const o=document.createElement('option'); o.value=y;o.textContent=y; year.appendChild(o); });
  function apply(){
    const q=(search.value||'').toLowerCase(), yf=year.value, s=sort.value;
    let list=pubs.filter(p=>{
      const text=[p.title,p.authors,p.venue].join(' ').toLowerCase();
      const okQ=!q||text.includes(q), okY=!yf||(String(p.year||'')===yf); return okQ&&okY;
    });
  ...
  search.addEventListener('input',apply); year.addEventListener('change',apply); sort.addEventListener('change',apply); apply();
})();
